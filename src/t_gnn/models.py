...
from t_gnn.ultra_models import EntityNBFNet, QueryNBFNet
...
class GNNRetriever(QueryGNN):
    ...
    def __init__(
        ...
        self.input_tri_proj = nn.Linear(3072, 512)
        self.input_rel_proj = nn.Linear(1024, 512)
    ...
    def forward(
        ...
        graph.rel_emb = self.input_rel_proj(graph.rel_emb)

        triple_embedding = 0
        for te in batch['question_embeddings']:
            triple_embedding += self.input_tri_proj(torch.cat([torch.tensor(x, device='cuda') for x in te], dim=0))
        triple_embedding /= len(batch['question_embeddings'])
        batch['question_embeddings'] = triple_embedding.unsqueeze(0)
